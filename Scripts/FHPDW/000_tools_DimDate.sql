...
IF OBJECT_ID('dbo.ufn_GetEasterSunday', 'FN') IS NULL EXEC ('CREATE FUNCTION dbo.ufn_GetEasterSunday (@a INT) RETURNS INT AS BEGIN RETURN @a; END;');
GO
...
DECLARE @EndDate DATE = '01/01/2021' --End Value of Date Range
...
FROM (
	...
	FROM (
		SELECT DISTINCT Year FROM dbo.DimDate
	) Y
) ES
INNER JOIN dbo.DimDate D ON D.Date = DATEADD(dd, -2, ES.EasterSunday);
...
INNER JOIN dbo.DimDate D ON D.Date = DATEADD(dd, 1, ES.EasterSunday);
